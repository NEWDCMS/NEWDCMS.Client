...
namespace Wesley.Client.ViewModels
{
    ...
    public class UpdatePageViewModel : ViewModelBase
    {
        ...
        [Reactive] public string DownloadInfoText { get; set; }
            ...
            IDialogService dialogService) : base(navigationService, dialogService)
        {
            ...
            this.WhenAnyValue(x => x.IsUpdating)
                //.Do(x => { _loggingService.Error("IsUpdating:", $"{IsUpdating}"); })
                .Select(x => x ? "下载更新" : "已经是最新版了")
                .Subscribe(x => this.DownloadInfoText = x).DisposeWith(DestroyWith);
