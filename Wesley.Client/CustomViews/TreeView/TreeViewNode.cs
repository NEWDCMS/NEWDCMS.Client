...
namespace Wesley.Client.CustomViews
{
    public class TreeViewNode : StackLayout
    {
        ...
        private readonly BoxView _EmptyBox = new BoxView { BackgroundColor = Color.Transparent, Opacity = .5 };
        ...
        internal readonly BoxView SelectionBoxView = new BoxView { Color = Color.Transparent, Opacity = .5, IsVisible = false };
        #endregion
        ...
        private void ChildSelected(TreeViewNode child)
        {
            ...
            if (child.BindingContext is XamlItem vm)
            {
                ...
                if (child.Content is StackLayout stacklayout)
                {
                    if (stacklayout.Children.Where(s => s.GetType().Name == "CheckBox").FirstOrDefault() is CheckBox check)
                        check.IsChecked = vm.Selected;
                }
            }
