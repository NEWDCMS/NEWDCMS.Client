    ...
    internal class TouchManipulationCanvasView : /*SKCanvasView*/ SKGLView, IDisposable
    {
        ...
        internal SKBitmap EditedBitmap
        {
            get
            {
                ...
                var (rect, scaleX, scaleY) = SkiaHelper.CalculateRectangle(rectInfo, outImageWidht, outImageHeight);
                using (SKCanvas canvas = new SKCanvas(outBitmap))
                {
                    OnPaintSurface(canvas, new SKRect(0, 0, outImageWidht, outImageHeight), false, -rect.Left, -rect.Top, 1 / scaleX);
                }
